...
Server::Server(const CustomSocket::IPEndpoint& IPconfig) :
	m_isRunning(false),
	m_getInfoEvent(CreateEventW(nullptr, TRUE, FALSE, L"ServiceEvent"))
{
	...
	if (result == CustomSocket::Result::Success)
	{
		...
		if (result == CustomSocket::Result::Success)
		{
			ConnectionService newConnectionService({
				...
				{nullptr, 0, 0, false},
				CreateEventW(nullptr, TRUE, FALSE, L"OnSendEvent"),
				{nullptr, 0, 0, false},
				CreateEventW(nullptr, TRUE, FALSE, L"OnRecieveEvent")
				});
